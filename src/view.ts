...
class ProjectTreeItem extends vscode.TreeItem {
  constructor(name: string, private projectTodos: todo.TodoItem[], viewType: todo.TodoView) {
    super(name);
    this.collapsibleState = (viewType == todo.TodoView.Anytime) ? vscode.TreeItemCollapsibleState.Expanded : vscode.TreeItemCollapsibleState.Collapsed
  }
...
export class TodoTreeView implements vscode.TreeDataProvider<TodoTreeItem> {
  ...
  private onDidChangeTreeDataEventEmitter: vscode.EventEmitter<TodoTreeItem | undefined> = new vscode.EventEmitter<TodoTreeItem | undefined>();
  ...
  async getChildren(element?: TodoTreeItem | ProjectTreeItem | undefined): Promise<TodoTreeItem[] | ProjectTreeItem[]> {
    ...
    if (this.type === todo.TodoView.Inbox || this.type === todo.TodoView.Today)
      return todos.map(todo => { return new TodoTreeItem(todo) })

    const projects = Array.from(new Set(todos.map(todo => todo.file)))
    return projects.map(project => new ProjectTreeItem(project, todos.filter(todo => todo.file === project), this.type))
